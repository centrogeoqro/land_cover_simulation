...
#Author:S. Ivvan Valdez and RODRIGO Lopez-Farias
...
def plotComp(T1, T2, TOCname1='TOC1', TOCname2='TOC2', title="default", filename='', ne = [0,0] , height = 1800, width = 1800, dpi = 300):
    ...
    if title=='default':
        ...
        plt.title(title)
    plt.text(0.2,0.025,'AUC-'+TOCname2+'='+str(round(T2['areaRatio'],4)))
 #   plt.text(-0.29+0.675,0.025,str(round(T2['areaRatio'],4)))
    plt.text(0.2,0.075,'AUC-'+TOCname1+'='+str(round(T1['areaRatio'],4)))
#    plt.text(-0.29+0.675,0.075,str(round(T1['areaRatio'],4)))
    #str(T1['areaRatio'])
    plt.plot(rx1, ry,'r--')
    plt.plot(rx2, ry,'b--')
    ...
    plt.plot((T1['TP+FP']/n1)[ne[0]],(T1['TP']/T1['npos'])[ne[0]], marker='o', linestyle='none', markersize=12, markeredgecolor='white', markerfacecolor='red', label = "Quantity: "+str(ne[0]))
    plt.plot((T2['TP+FP']/n2)[ne[1]],(T2['TP']/T2['npos'])[ne[1]],marker='o', linestyle='none', markersize=12, markeredgecolor='white',markerfacecolor='blue',label = "Quantity: "+str(ne[1]))
...
def plot(T, filename = '', title='default', TOCname='TOC', normalized=False,
         height=1800, width=1800, dpi=300, ne = 50):
    ...
    if (T['type']=='TOC'):
        ...
        if(normalized):        
            ...
            plt.plot(rx, ry,'b--')
            plt.plot(T['TP+FP']/T['ndata'],T['TP']/T['npos'],'r-', 
                     label=TOCname, 
                     linewidth=3)
            plt.plot(px,py)
        else:
    ...
    if (filename!=''):
        ...
        #plt.ylabel("Hits")
        #if (title=='default'):
                #title="Difference of two TOC curves"
        #plt.title(title)
        #plt.text(0,0.85*T['npos'],'AUC-'+TOCname2+'=')
        #plt.text(0+0.1*T['ndata'],0.85*T['npos'],str(T['areaRatio']))
        #plt.plot(x, y,'b--')
        #plt.plot(T['TP+FP'],T['TP'],'r-',label="TOC-Diff")
        #plt.legend()
